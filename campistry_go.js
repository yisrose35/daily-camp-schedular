...
(function () {
    ...
    async function _fetchRoadGeometry(item) {
        ...
        try {
            let osrmPairs = pairs;
            if (pairs.length > 100) {
                const step = (pairs.length - 1) / 99;
                osrmPairs = Array.from({ length: 100 }, function(_, i) {
                    return pairs[Math.round(i * step)];
                });
            }
    ...
    async function fetchOSRMWithRetry(coordStr, retries) {
        ...
        var now = Date.now(), gap = now - _osrmLastCall;
        if (gap < 200) await new Promise(function(r) { setTimeout(r, 200 - gap); });
        _osrmLastCall = Date.now();
        for (let attempt = 0; attempt < retries; attempt++) {
            if (attempt > 0) {
                const delay = 2000 * Math.pow(2, attempt); // 2s, 4s, 8s backoff for 429s
    ...
    async function fetchTrafficLegs(orderedStops, campLat, campLng, departureTimeMin, isArrival) {
        ...
        try {
            ...
            const resp = await fetch(url);
            if (resp.status === 429) { await new Promise(function(r) { setTimeout(r, 3000); }); return null; }
    ...
    async function generateRoutes() {
        ...
        for (let si = 0; si < shifts.length; si++) {
            ...
            for (let zi = 0; zi < greedyZones.length; zi++) {
                ...
                const zoneStops = zone.stopIndices.map(function(si) { return allStops[si]; });
                const v = shiftVehicles.find(function(sv) { return sv.busId === zone.busId; }) || shiftVehicles[0];
                ...
                let fIdx = 0, fDist = 0;
                zoneStops.forEach(function(s, i) { var d = drivingDist(campLat, campLng, s.lat, s.lng); if (d > fDist) { fDist = d; fIdx = i; } });

                let orderedStops = null;

                // VROOM per-zone TSP
                if (key && zoneStops.length > 1) {
                    var jobs = zoneStops.map(function(stop, i) {
                        return { id: i + 1, location: [stop.lng, stop.lat], service: serviceTime, amount: [stop.campers.length], description: stop.address };
                    });
                    var veh = { id: 1, profile: 'driving-car', capacity: [v.capacity], description: v.name };
                    if (isArrival) { veh.start = [zoneStops[fIdx].lng, zoneStops[fIdx].lat]; veh.end = [campLng, campLat]; }
                    else { veh.start = [campLng, campLat]; veh.end = [zoneStops[fIdx].lng, zoneStops[fIdx].lat]; }
                    try {
                        var resp = await fetch('https://api.openrouteservice.org/optimization', {
                            ...
                            body: JSON.stringify({ jobs: jobs, vehicles: [veh] })
                        });
                        if (resp.ok) {
                            var result = await resp.json();
                            var vroomRoute = result.routes?.[0];
                            if (vroomRoute) {
                                var ordered = [];
                                vroomRoute.steps.forEach(function(step) { if (step.type !== 'job') return; var stop = zoneStops[step.id - 1]; if (stop) ordered.push(stop); });
                                if (result.unassigned?.length) { result.unassigned.forEach(function(ua) { var stop = zoneStops[ua.id - 1]; if (stop) cheapestInsert(ordered, stop); }); }
                ...
                if (orderedStops.length >= 2) {
                    ...
                    if (isArrival && fd < ld) orderedStops.reverse();
                    if (!isArrival && fd > ld) orderedStops.reverse();
                }

                var routeStops = orderedStops.map(function(stop, i) { return { stopNum: i + 1, campers: stop.campers, address: stop.address, lat: stop.lat, lng: stop.lng }; });
                routes.push({
            ...
            function _estRouteTime(routeObj) {
                ...
                var t = drivingDist(campLat, campLng, routeObj.stops[0].lat, routeObj.stops[0].lng) + serviceTime;
                for (var ri = 1; ri < routeObj.stops.length; ri++) {
                    t += drivingDist(routeObj.stops[ri - 1].lat, routeObj.stops[ri - 1].lng, routeObj.stops[ri].lat, routeObj.stops[ri].lng) + serviceTime;
                }
                return t;
            }
            ...
            for (var xp = 0; xp < 100; xp++) {
                ...
                for (var xli = 0; xli < routes[slowRi].stops.length; xli++) {
                    ...
                    for (var xti = 0; xti < routes.length; xti++) {
                        ...
                        var newDonorTime = 0, newRecvTime = 0;
                        if (donorStops2.length) { newDonorTime = drivingDist(campLat, campLng, donorStops2[0].lat, donorStops2[0].lng) + serviceTime; for (var dk = 1; dk < donorStops2.length; dk++) { newDonorTime += drivingDist(donorStops2[dk - 1].lat, donorStops2[dk - 1].lng, donorStops2[dk].lat, donorStops2[dk].lng) + serviceTime; } }
                        newRecvTime = drivingDist(campLat, campLng, recvStops2[0].lat, recvStops2[0].lng) + serviceTime;
                        for (var rk = 1; rk < recvStops2.length; rk++) { newRecvTime += drivingDist(recvStops2[rk - 1].lat, recvStops2[rk - 1].lng, recvStops2[rk].lat, recvStops2[rk].lng) + serviceTime; }

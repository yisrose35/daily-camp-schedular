...
(function() {
            ...
            try {
                ...
                if (value !== null && _storageChecksums[key]) {
                   ...
                   if (currentChecksum !== _storageChecksums[key]) {
                        ...
                        // ★★★ v1.1 FIX: Accept all protected-key changes from app code ★★★
                        // During signup landing.js writes role='owner'. This is NOT tampering.
                        // Write-time verification (verifyBeforeWrite) is the real security gate.
    ...
    function injectCSP() {
        ...
        const csp = [
            ...
            "connect-src 'self' https://bzqmhcumuarrbueqttfh.supabase.co wss://bzqmhcumuarrbueqttfh.supabase.co https://fonts.googleapis.com https://fonts.gstatic.com https://cdn.jsdelivr.net",
            "base-uri 'self'",
            "form-action 'self'"
        ].join('; ');

...
(function () {
    ...
    async function runBunkGeneration(dateStr, daAutoLayers, onComplete, onError) {
        ...
        try {
            ...
            // CRITICAL: config must include masterFields + masterSpecials so
            // buildAllCandidateOptions() can populate the candidate pool.
            // Without these, the solver has nothing to assign and returns "Free".
            var gs = window.loadGlobalSettings ? window.loadGlobalSettings() : {};
            var masterFields = (gs.app1 && gs.app1.fields) || gs.fields ||
                               window.fields || [];
            var masterSpecials = (gs.app1 && gs.app1.specialActivities) || gs.specialActivities ||
            ...
            // ── 8. Run the solver ─────────────────────────────────────────
            // Set _autoBuildRunActive so division_times_integration.js bypasses
            // its DT rebuild wrapper and scheduler_core_main doesn't re-stamp
            // pinned events from a skeleton that doesn't exist in bunk mode.
            window._autoBuildRunActive = true;
            window._bunkModeActive = true;
            log('Handing off to Solver.solveSchedule()...');
            Solver.solveSchedule(activityBlocks, config);
            window._autoBuildRunActive = false;
    ...
    function buildActivityBlocks(bunkTimelines, perBunkDivisionTimes) {
        ...
        Object.values(bunkTimelines).forEach(function (tl) {
            ...
            tl.slots.forEach(function (slot) {
                ...
                // Map activityType → the exact event strings the solver's domain builder
                // uses for candidate filtering (v15.1 event-type filtering):
                //   'sports slot'      → sports candidates only
                //   'special activity' → special candidates only
                //   anything else      → all candidates
                var solverEvent = (type === 'sports' || type === 'sport')
                    ? 'Sports Slot'
                    : (type === 'special' || type === 'specials')
                        ? 'Special Activity'

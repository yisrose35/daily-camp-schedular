...
(function() {
    ...
    // ★★★ DAY 16b DEBUG: hydration timeline tracer ★★★
    // Records window.scheduleAssignments activity count every 250ms for 40s
    // after page load. Persists to localStorage so it survives reload. Inspect
    // via window.__campDumpHydrateTimeline() in console after reload completes.
    // Remove this block once Day 16b is resolved.
    try {
        const TRACE_KEY = '__campistry_hydrate_trace';
        const trace = [];
        const start = performance.now();
        const sample = (label) => {
            const sa = window.scheduleAssignments || {};
            let acts = 0, nullSlots = 0, totalSlots = 0, activeBunks = 0;
            const perBunk = {};
            for (const bk of Object.keys(sa)) {
                ...
                for (const s of arr) {
                    ...
                    if (s && typeof s === 'object') {
                        const act = s._activity || s.activity;
                        if (act && act !== 'Free') {
                            acts++;
                            bunkActs++;
                        }
                ...
                if (bunkActs > 0) activeBunks++;
                perBunk[bk] = bunkActs;
            }
            trace.push({
                t: Math.round(performance.now() - start),
                label,
                bunks: Object.keys(sa).length,
                acts,
                nullSlots,
                totalSlots,
                activeBunks,
                perBunkActs: Object.fromEntries(Object.entries(perBunk).slice(0, 3))
            });
            try { localStorage.setItem(TRACE_KEY, JSON.stringify(trace)); } catch (_) {}
        };
        sample('init');
        const intervalId = setInterval(() => sample('tick'), 250);
        const events = [
            'campistry-cloud-hydrated',
            'campistry-schedule-loaded',
            'campistry-generation-complete',
            'campistry-orchestrator-ready',
            'campistry-rbac-ready'
        ];
        events.forEach(ev => window.addEventListener(ev, () => sample('event:' + ev)));
        setTimeout(() => { clearInterval(intervalId); sample('end'); }, 40000);
        window.__campDumpHydrateTimeline = () => {
            const t = JSON.parse(localStorage.getItem(TRACE_KEY) || '[]');
            console.log('═══════ Hydrate Timeline (' + t.length + ' samples) ═══════');
            let prev = null;
            t.forEach(s => {
                const marker = s.label === 'tick' ? '·' : '→';
                const diff = (prev !== null && prev !== s.acts) ? ` (${s.acts > prev ? '+' : ''}${s.acts - prev})` : '';
                console.log(marker, s.t + 'ms', '[' + s.label + ']', 'bunks=' + s.bunks, 'acts=' + s.acts + diff, 'activeBunks=' + s.activeBunks);
                prev = s.acts;
            });
            return t;
        };
    } catch (_e) { console.warn('[hydrate-trace] init failed', _e); }

...
## 9. Navigating to an older date rebuilt the day only partially

The cloud merge (`ScheduleDB.mergeSchedules`) returns the **complete** day:

```
scheduleAssignments · scheduleSegments · leagueAssignments · unifiedTimes
divisionTimes (deserialized) · _perBunkSlotsData · _autoGenerated · manualSkeleton
· isRainyDay · rainyDayStartTime
```

`loadSchedule` also writes that whole payload to localStorage via `setLocalSchedule`, so
everything needed to redraw the day is present locally the moment the load resolves. The
date-change handler in `integration_hooks.js` then applied **five** of those fields and
dropped the rest.

| Dropped | Consequence |
|---|---|
| `scheduleSegments` | the segment store kept the **previous** date's cells — the lag `analytics.js` documents and works around |
| `_perBunkSlotsData` | an auto-built day lost its per-bunk grid geometry and rendered with `+ Add` gaps where activities were |
| `divisionTimes` deserialization | assigned **raw**; `_perBunkSlots` is a custom array property JSON drops, so the geometry was gone even when the payload carried it |
| `_autoGenerated` / `manualSkeleton` | downstream code couldn't tell an auto build from a manual one for that day |
| `window._autoSkeleton` | the **save side** reads this one. Navigating from auto-built day A to day B left it holding A's skeleton, so the next lazily-built save of B attached **A's skeleton to B's cloud row** |

Two more branches were wrong:

- **A transient load failure had no branch at all.** Memory kept the previous date's
  schedule while `currentScheduleDate` had already advanced — the user was shown yesterday's
  day under today's label. (The save guards from section 6 then correctly refused to persist
  it, so the data was safe; what was on screen was not.)
- **The empty-date branch cleared only assignments and leagues**, so an empty date still
  drew the prior day's grid geometry and segments.

**Fixed:** a single `applyLoadedDay` applies every field and then delegates the geometry
restore to `unified_schedule_system`'s `loadScheduleForDate` — the one function that already
reattaches `_perBunkSlots` with auto/manual mode isolation, realigns each bunk's assignments
onto the slot windows (so a day saved at an older grid shape doesn't render one position
off), and falls back to the skeleton. Rather than grow a second divergent restore, the
handler now calls the correct one.

`clearDayInMemory` clears **every** per-day global — assignments, leagues, segments,
`_autoSkeleton`, and per-bunk geometry — and the failure path falls back to the local copy of
that date, or clears, so another day's data is never displayed as this one.

The orchestrator's `hydrateWindowGlobals` had the same segment gap (it was the only per-day
global it never touched) and its no-data branch cleared only assignments and leagues. Both
fixed to match.

**Test:** `tests/date_change_rebuild.test.js` — 12 cases over the real `applyLoadedDay` /
`clearDayInMemory` and the branch selection. Verified to have teeth: 7 of the 12 fail against
the pre-fix code.

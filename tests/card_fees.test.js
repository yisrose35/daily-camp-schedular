...
test('the charge records what it was a fee ON, and increases the balance', () => {
    ...
    // ...and which card it was on (TED-147), the default one Charge Card uses
    assert.match(body, /cardFee:\{mode:q\.mode,base:Math\.round\(base\*100\)\/100,reason:q\.reason,funding:_familyCardFunding\(f\),\s*token:\(_familyCardOnFile\(f\)\|\|\{\}\)\.token\|\|null\}/);

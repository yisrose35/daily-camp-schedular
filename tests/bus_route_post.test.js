...
test('a short bus full of near-camp children is emptied into the buses that pass them and refilled with a far branch', () => {
    ...
    // C1 and C2 have no stop within 5mi of E's — they drive out through the
    // core without stopping — so the reach rule alone would never let them
    // take E's children; a stop near camp is on every bus's way out.
    ...
    const C1 = [at(7, 1, 8), at(9, 1.5, 8), at(11, 2, 6)];
    const C2 = [at(7, -1, 8), at(9, -1.5, 8), at(11, -2, 6)];
    ...
    const lens = res.buckets.map(solo);
    assert.ok(res.blockMoves + res.refills >= 1, 'a branch moved: ' + JSON.stringify({ moves: res.moves, blockMoves: res.blockMoves, refills: res.refills }));
    assert.ok(Math.max(...lens) < aBefore - 10, 'the long run came down: ' + lens.map(x => x.toFixed(0)).join(', '));
    assert.ok(Math.min(...lens) > 35, 'no bus is left idle: ' + lens.map(x => x.toFixed(0)).join(', '));
    assert.ok(res.buckets[0].some(a => P.haversineMi(CAMP.lat, CAMP.lng, a.lat, a.lng) > 4), 'the short bus now goes out: ' + res.buckets[0].map(a => a.address).join(' | '));

...
function buildForGrade(params) {
    ...
    // ★ FIX: Write bunk overrides so solver cannot overwrite swim slots
    bunks.forEach(function(bunk) {
        bunkOverrides.push({
            bunk: bunk, division: divName,
            activity: piece.eventName, type: 'pinned',
    ...
    if (changeDur > 0) {
        if (bestPos.endMin + changeDur <= divTimes.end) {
            ...
            // ★ FIX: Change buffer also needs bunk overrides
            bunks.forEach(function(bunk) {
                bunkOverrides.push({
                    bunk: bunk, division: divName,
                    activity: 'Change', type: 'pinned',
                    startTime: fmtTime(bestPos.endMin), endTime: fmtTime(bestPos.endMin + changeDur),
                    _autoGenerated: true, _fixedEvent: true
                });
            });
        }
        if (bestPos.startMin - changeDur >= divTimes.start) {
            ...
            if (!divisionOccupied.some(function(o) { return overlaps(preS, bestPos.startMin, o.startMin, o.endMin); })) {
                ...
                // ★ FIX: Pre-change buffer also needs bunk overrides
                bunks.forEach(function(bunk) {
                    bunkOverrides.push({
                        bunk: bunk, division: divName,
                        activity: 'Change', type: 'pinned',
                        startTime: fmtTime(preS), endTime: fmtTime(bestPos.startMin),
                        _autoGenerated: true, _fixedEvent: true
                    });
                });
            }

...
function buildForGrade({ gradeName, divName, bunks, layers, dayName, dateStr, divTimes, scarceSpecials, regularSpecials, todaysSpecials, warnings }) {
    ...
    bunks.forEach(bunk => {
        ...
        for (const gap of gapsCopy) {
            ...
            // Fill remaining gap with sports
            if (remaining > 0 && sportsNeeded > 0) {
                const blockStart = cursor;
                const blockEnd = gap.endMin; // sports fill to end of gap
                ...
                skeleton.push({
                    ...
                    _autoGenerated: true,
                    _bunk: bunk
                    // NO _durationStrict — any sport can fill any duration
                });
                ...
                log(`      ${bunk}: Sport slot ${fmtTime(blockStart)}-${fmtTime(blockEnd)}`);
            }
            
            // If still remaining gap and no specific needs, fill with general activity
            if (remaining >= 15) { // minimum 15min block
                const blockStart = cursor;
                const blockEnd = gap.endMin;

...
function buildForGrade({ gradeName, divName, bunks, layers, dayName, dateStr, divTimes, scarceSpecials, regularSpecials, todaysSpecials, warnings, globalScarceUsage }) {
    ...
    sortedFixed.forEach(layer => {
        ...
        // ★★★ v3.2.9 FIX #9b: Per-bunk scarce check + GUARANTEED bunk overrides ★★★
        // Fixed events (lunch/snack/dismissal) MUST appear for every bunk. The skeleton
        // pinned path (Step 3) is unreliable — slot grid fragmentation and _bunkOverride
        // conflicts cause the solver to skip filling snacks for some bunks.
        // Solution: create a bunk override for EVERY bunk. Step 2 processes these first,
        // setting _bunkOverride which protects the slot from being overwritten.
        ...
        bunks.forEach(bunk => {
            ...
            if (hasConflict && isLunchOrSnack) {
                // This bunk has a scarce activity during the fixed event — find alternative
                const altPlacement = findPerBunkFixedPlacement(
                    bunk, bunkState[bunk].occupied, eventName, duration,
                    windowStart, windowEnd, divTimes
                );
                ...
                // No conflict — standard placement + bunk override for guaranteed fill
                bunkState[bunk].occupied.push({
                    startMin: placement.startMin, endMin: placement.endMin,
                    event: eventName, type: 'fixed'
                });
                ...
                // ★★★ v3.2.9 FIX #9b: Bunk override guarantees Step 2 fills this ★★★
                if (isLunchOrSnack) {
                    bunkOverrides.push({
                        bunk, division: divName, activity: eventName, type: 'pinned',
                        startTime: fmtTime(placement.startMin),
    ...
    // ═════════════════════════════════════════════════════════════════
    // PHASE 5: Sort timelines + reorder bunk overrides
    // ═════════════════════════════════════════════════════════════════
    ...
    // ★★★ v3.2.9 FIX #9b: Fixed event overrides LAST in array ★★★
    // scheduler_core_main Step 2 processes overrides sequentially.
    // fillBlock overwrites existing assignments. By placing fixed event
    // overrides (lunch/snack/dismissal) LAST, they overwrite any knapsack
    // overrides that claimed the same slot, guaranteeing every bunk has
    // its lunch/snack/dismissal.
    const fixedOverrides = [];
    const otherOverrides = [];
    bunkOverrides.forEach(o => {
        if (o._fixedEvent || o._perBunkFixed || /lunch|snack|dismissal/i.test(o.activity)) {
            fixedOverrides.push(o);
        } else {
            otherOverrides.push(o);
        }
    });
    // Clear and refill: scarce first, then knapsack, then fixed events last
    bunkOverrides.length = 0;
    bunkOverrides.push(...otherOverrides, ...fixedOverrides);

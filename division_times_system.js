// =============================================================================
// division_times_system.js v1.5 — CAMPISTRY PER-DIVISION TIME SLOT SYSTEM
// =============================================================================
//
...
if (hasBunkSpecificBlocks) {
    ...
    // ★★★ v1.5 FIX: Build _perBunkSlots when skeleton is all-per-bunk
    // When auto core produces only per-bunk blocks, divWideBlocks is empty,
    // meta is always undefined, and _perBunkSlots never gets populated.
    // Group directly by _bunk to produce correct per-bunk slot arrays.
    const allBunksInDiv = [...new Set(withExpandedSplits.map(b => b._bunk).filter(Boolean))];
    if (allBunksInDiv.length > 0) {
        const perBunkSlots = {};
        allBunksInDiv.forEach(bunkId => {
            const bunkBlocks = withExpandedSplits
                .filter(b => b._bunk === bunkId)
                .sort((a, b) => a.startMin - b.startMin);
            perBunkSlots[String(bunkId)] = bunkBlocks.map((b, idx) => ({
                slotIndex: idx,
                startMin: b.startMin,
                endMin: b.endMin,
                ...
                _classification: b._classification || null,
                _fixed: b._fixed || false,
                _pinned: b._pinned || false,
        ...
        const maxLen = Math.max(...Object.values(perBunkSlots).map(s => s.length));
        slotsForDiv._isPerBunk = true;
        slotsForDiv._perBunkSlots = perBunkSlots;
        slotsForDiv.length = maxLen;
        log(`  ${divName}: per-bunk mode — ${allBunksInDiv.length} bunks, max ${maxLen} slots each`);
    }
} else {
    ...
    // ★★★ v1.5 FIX (single-bunk): If all blocks belong to one bunk and no
    // overlaps exist (single bunk never triggers hasBunkSpecificBlocks),
    // still build _perBunkSlots so Step 2.7 can initialize scheduleAssignments.
    const allBunksInDiv = [...new Set(withExpandedSplits.map(b => b._bunk).filter(Boolean))];
    if (allBunksInDiv.length > 0) {
        const perBunkSlots = {};
        allBunksInDiv.forEach(bunkId => {
        ...
        slotsForDiv._perBunkSlots = perBunkSlots;
        slotsForDiv.length = maxLen;
        log(`  ${divName}: per-bunk mode (single-bunk) — ${allBunksInDiv.length} bunk, ${maxLen} slots`);
    }
}
// Create slot array

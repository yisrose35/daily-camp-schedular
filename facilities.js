...
function summarySpecialMultiPart(s) {
    ...
    const hasNames = Array.isArray(s.multiPart.parts) && s.multiPart.parts.some(p => p.name);
    const nameStr = hasNames ? ' (' + s.multiPart.parts.map((p,i) => p.name || ('Part '+(i+1))).join(', ') + ')' : '';
    return `${s.multiPart.totalParts} parts, ${s.multiPart.daysBetween}d apart${nameStr}`;
}
...
function renderSpecialMultiPart(saData) {
    ...
    const renderContent = () => {
        ...
        toggleRow.style.cssText = "display:flex; align-items:center; gap:10px; margin-bottom:12px;";
        const tog = document.createElement("label"); tog.className = "switch";
        const cb = document.createElement("input"); cb.type = "checkbox"; cb.checked = mp.enabled;
        cb.onchange = () => { mp.enabled = cb.checked; saveSpecialData(saData); renderContent(); updateSummary(); };
        const sl = document.createElement("span"); sl.className = "slider";
        tog.appendChild(cb); tog.appendChild(sl);
        ...
        const partsLbl = document.createElement("span"); partsLbl.style.cssText = "font-size:0.85rem; font-weight:500;"; partsLbl.textContent = "Number of parts:";
        const partsIn = document.createElement("input"); partsIn.type = "number"; partsIn.min = "2"; partsIn.max = "10"; partsIn.value = mp.totalParts || 2;
        ...
        const daysLbl = document.createElement("span"); daysLbl.style.cssText = "font-size:0.85rem; font-weight:500;"; daysLbl.textContent = "Days between parts:";
        const daysIn = document.createElement("input"); daysIn.type = "number"; daysIn.min = "1"; daysIn.max = "30"; daysIn.value = mp.daysBetween || 3;
        daysIn.style.cssText = "width:60px; padding:4px; border-radius:6px; border:1px solid #D1D5DB; text-align:center;";
        daysIn.onchange = () => { mp.daysBetween = Math.max(1, Math.min(30, parseInt(daysIn.value) || 3)); saveSpecialData(saData); updateSummary(); };
        const daysNote = document.createElement("span"); daysNote.style.cssText = "font-size:0.8rem; color:#6B7280;"; daysNote.textContent = "days gap required between each part";
        ...
        const partsNote2 = document.createElement("span");
        ...
        for (let i = 0; i < (mp.totalParts || 2); i++) {
            const part = mp.parts[i] || { name: '', location: '' };
            ...
            nameInp.onchange = () => { mp.parts[i].name = nameInp.value.trim(); saveSpecialData(saData); };
            ...
            locInp.onchange = () => { mp.parts[i].location = locInp.value.trim(); saveSpecialData(saData); };
...
function renderSpecialMultiPart(saData) {
    ...
    const renderContent = () => {
        ...
        // Toggle row — build purely with DOM to avoid innerHTML += destroying listeners
        const toggleRow = document.createElement("div");
        toggleRow.style.cssText = "display:flex; align-items:center; gap:10px; margin-bottom:12px;";
        const tog = document.createElement("label"); tog.className = "switch";
        const cb = document.createElement("input"); cb.type = "checkbox"; cb.checked = mp.enabled;
        ...
        daysIn.style.cssText = "width:60px; padding:4px; border-radius:6px; border:1px solid #D1D5DB; text-align:center;";
        daysIn.onchange = () => {
            mp.daysBetween = Math.max(1, Math.min(30, parseInt(daysIn.value) || 3));
            saveSpecialData(saData); updateSummary();
        };
        const daysNote = document.createElement("span"); daysNote.style.cssText = "font-size:0.8rem; color:#6B7280;"; daysNote.textContent = "days gap required between each part";
        daysRow.appendChild(daysLbl); daysRow.appendChild(daysIn); daysRow.appendChild(daysNote);
        ...
        // Per-part name + location
        const partsHeader = document.createElement("div");
        partsHeader.style.cssText = "font-size:0.85rem; font-weight:600; color:#374151; margin-top:4px;";
        partsHeader.textContent = "Part Details";
        const partsNote = document.createElement("span");
        partsNote.style.cssText = "font-size:0.75rem; font-weight:400; color:#6B7280; margin-left:6px;";
        partsNote.textContent = "(optional: custom name & location)";
        partsHeader.appendChild(partsNote);
        config.appendChild(partsHeader);

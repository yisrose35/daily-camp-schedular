...
        
        // Use the globally defined constant from the top of the file
        const slotEnd = slotStart + INCREMENT_MINS;
        
        // Correct "contains" check: (slotStart <= startMin < slotEnd)
        // This finds the slot that the event's start time *falls into*.
...
Additional             td.className = "grey-cell";
            tr.appendChild(td);
...
CC                         if (entry && entry._h2h && entry.sport) {
...
                        html = `<p class="muted" style="margin:0; padding: 4px;">${eventBlock.event}</p>`;
                    } else {
                    C   html = `<p style="margin:2px 0 5px 4px; font-weight: bold;">${eventBlock.event}</p>`;
...
        
  Indentation       const savedTimes = data.unifiedTimes || [];

...
--   4. A refund not on these books is left alone; a browser can call none of it.
...
BEGIN
    ...
    IF n <> 1 THEN RAISE EXCEPTION 'no Billing notice for the failed canteen refund'; END IF;
    ...
    r := public.reverse_failed_stripe_refund(c, 're_never', NULL);
    IF r->>'error' IS DISTINCT FROM 'refund_not_found' THEN RAISE EXCEPTION 'an unknown refund: %', r; END IF;
    IF has_function_privilege('authenticated', 'public.reverse_failed_stripe_refund(uuid,text,text)', 'EXECUTE')
       OR has_function_privilege('anon', 'public.reverse_failed_stripe_refund(uuid,text,text)', 'EXECUTE') THEN

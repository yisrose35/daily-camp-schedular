...
#
# They live in scripts/pgstubs.sql rather than in a heredoc here, because the
# browser end-to-end harness (tests/e2e/db.js) boots the same database and two
# copies would drift.
STUBS_SQL="$(dirname "$0")/pgstubs.sql"
[ -f "$STUBS_SQL" ] || { echo "missing $STUBS_SQL" >&2; exit 1; }
$PSQL -f "$STUBS_SQL" >"$BASE/stubs.log" 2>&1 || { echo "stubs failed:" >&2; cat "$BASE/stubs.log" >&2; exit 1; }

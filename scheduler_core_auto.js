       ...
       // ── STEP 2.3 ROUND-ROBIN PLACEMENT ENGINE ──
        ...
        // (delete the old closing `}` for bunk loop and layer loop here)
               for (const layer of gradeLayers) {
                    ...
                    if (type === 'special') {
                        // ── SPECIAL LAYER ──
                        // Pull from ranked queue, check capacity
                        // Keep pulling if operator >= and quantity > 1, stop if = 1

                        const usedExclusions = new Set(
                            Object.keys(bunkSpecialAssigned[bunk] || {})
                        );
                       ...
                       while (placedCount < requiredCount) {
                            ...
                            placeTentativeBlock(bunk, {
                                ...
                                _committed: false // tentative until Step 2.6
                            });

                            placedCount++;
                        }

                        // If operator is >= and quantity > 1, keep filling the window
                        if ((op === '>=' || op === '≥') && requiredCount > 1) {
                            ...
                            while (keepFilling) {
                               ...
                               claimSpecial(bunk, nextCandidate, nextPosition.start, nextPosition.end);
                                ...
                                placeTentativeBlock(bunk, {
                                    startMin: nextPosition.start,
                                    endMin: nextPosition.end,
                                    type: 'special',
                                    event: nextCandidate.name,
                                    layer,
                                    _classification,
                                    _assignedSpecial: nextCandidate.name,
                                    _specialDuration: nextCandidate.duration,
                                    _specialLocation: nextCandidate.location,
                                    _activityLocked: true,
                                    _bunkOverride: true,
                                    _isScarce: nextCandidate.isScarce,
                                    _committed: false
                                });
                            }
                        }
        ...
        if (window.DivisionTimesSystem) {
            ...
            err('[STEP 2.7] DivisionTimesSystem not available — cannot build _perBunkSlots');
                ...
                timeline
                    ...
                    .forEach(block => {
                        ...
                        schedulableSlotBlocks.push({
                            ...
                            event: block.event || 'General Activity Slot',
                            type: block.type || 'slot',
                            startTime: block.startMin,
                            endTime: block.endMin,
                            slots: [slotIdx],

...
(function () {
    ...
    window.runAutoScheduler = async function (layers, options) {
        ...
        function perfectFitDistribute(blockDescs, target) {
            ...
            durations[n - 1] = Math.max(blockDescs[n - 1].dMin, target - total);
            return durations;
        }
        ...
        const leagueBlocks = (() => {
            ...
            return autoSkeleton.filter(b => {
                ...
                slots: (() => { const ds = window.divisionTimes?.[b.division]; if (!Array.isArray(ds)) return []; const idx = ds.findIndex(s => s.startMin === b.startMin); return idx >= 0 ? [idx] : []; })(),
                _autoGenerated: true
            }));
        })();
        ...
        if (window.saveCurrentDailyData) {
            try {
                ...
                Object.entries(window.scheduleAssignments || {}).forEach(([b, s]) => {
                    clean[b] = (s || []).map(x => (x && x.field === 'Free' && !x._fixed) ? null : x);
                });
                const spbs = {};
                ...
                Object.assign(allDaily[dateKey], {
                    ...
                    _generatedAt: new Date().toISOString(), _warnings: warnings,
                ...
                if (typeof window.saveGlobalSettings === 'function') window.saveGlobalSettings('daily_schedules', allDaily);
                log('[5] Saved');

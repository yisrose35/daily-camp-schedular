...
// =====================================================================
// PHASE 2: GLOBAL PLANNER (replaces old runDraft)
// =====================================================================
// Sees ALL bunks across a grade simultaneously. For each time window
// where bunks compete for fields, it counts supply vs demand and
// allocates specials to bunks that would otherwise be stuck.
//
// Output: draftResults — same format as old runDraft:
//   { [bunk]: { sports: [], specials: [], elective: [], generic: [],
//               usedActivities: Set, grade: string } }
// =====================================================================
...
function runGlobalPlanner(shoppingLists) {
    ...
    // Track special assignments globally (cross-grade)
    const globalSpecialUsage = {}; // specialName → [{ grade, startMin, endMin }]

    function canAssignSpecialToGrade(specialName, grade, startMin, endMin) {
        const info = getSpecialSharingInfo(specialName, activityProperties, globalSettings);
        const existing = globalSpecialUsage[specialName] || [];
        for (const e of existing) {
            if (e.endMin <= startMin || e.startMin >= endMin) continue; // no overlap
            if (e.grade === grade) {
    ...
    allGrades.forEach(grade => {
        ...
        if (bunks.length <= 1) {
            // Single bunk: no competition, use simple draft
            bunks.forEach(bunk => {
        ...
        const bunkGaps = {}; // bunk → [{ start, end, duration }]
        const sportMinDur = {}; // bunk → minimum sport duration
        ...
        // Find all unique time boundaries from all bunk gaps
        ...
        // Build demand windows: time slices where 2+ bunks have gaps
        const demandWindows = [];
        for (let i = 0; i < sortedBounds.length - 1; i++) {
            ...
            // Which bunks have a free gap covering this slice?
        ...
        // Merge adjacent windows with the same bunk set
        const mergedWindows = [];
        ...
        mergedWindows.forEach(win => {
            // Field supply: count same-division capacity for this grade
            let fieldSupply = 0;
            const availableFields = []; // [{ name, capacity, remaining, activities }]
        ...
        // Sort by tightness: highest deficit first, then lowest supply ratio
        ...
        // Track what each bunk is assigned across windows
        const bunkDoneToday = {}; // bunk → Set of activity names
        const bunkAssignedWindows = {}; // bunk → [{ start, end, type, ... }]
        bunks.forEach(bunk => {
            bunkDoneToday[bunk] = new Set();
            bunkAssignedWindows[bunk] = [];
        });

        // Track field claims made by the planner
        const plannerFieldClaims = []; // [{ field, start, end, bunk, grade, activity }]

        function isFieldStillAvailable(fieldName, startMin, endMin, bunk) {
            // Check against both fieldLedger claims AND planner claims
            if (!isFieldAvailable(fieldName, startMin, endMin, bunk, grade)) return false;
            // Also check planner's own claims (not yet in fieldLedger)
            const ledger = fieldLedger[fieldName];
            if (!ledger) return false;
        ...
        function claimFieldForPlanner(fieldName, startMin, endMin, bunk, activity) {
            // Register in the actual field ledger so subsequent checks see it
        ...
        for (const win of windowsToProcess) {
            // Filter to bunks that haven't already been assigned in this window
            ...
            // Recompute field supply (may have changed from prior window allocations)
            ...
            if (deficit <= 0) {
                // Enough fields for everyone → assign sports
                assignSportsToWindow(win, unassignedBunks, currentFields, grade);
            } else {
                // Not enough fields → some bunks need specials
                // Score bunks: which ones are most flexible (can get sports elsewhere)?
                const bunkFlexibility = unassignedBunks.map(bunk => {
                    const sl = shoppingLists[bunk];
                    const otherWindows = (bunkGaps[bunk] || []).filter(g =>
                        // Has gap NOT overlapping this window
                ...
                // Most flexible bunks → get specials (they can get sports in other windows)
                ...
                // Also handle case where we can't divert enough to specials
                // (not enough specials available) — those overflow bunks become sport
                // and will compete; the solver handles remaining Frees
                ...
                // Assign specials
                assignSpecialsToWindow(win, specialBunks, grade);

                // Assign sports to the rest
                assignSportsToWindow(win, sportBunks, currentFields, grade);
            }
        }

        // ── Handle single-bunk windows and remaining needs ────────
        bunks.forEach(bunk => {
            const sl = shoppingLists[bunk];
            if (!sl) return;
            const result = draftResults[bunk];

            // Fill remaining specials (not yet assigned by planner)
            ...
            // Fill remaining sports (not yet assigned by planner)
    ...
    function assignSportsToWindow(win, bunkList, availFields, grade) {
        // Sort bunks by rotation need (most constrained first)
        ...
        // Build a copy of available field slots we can decrement
        ...
        for (const { bunk } of scoredBunks) {
            ...
            // Try to find a sport + field combo
            for (const sport of (sl.sports?.priorityList || [])) {
                ...
                for (const fs of fieldSlots) {
                    ...
                    // Verify field is actually available at this time
    ...
    function assignSpecialsToWindow(win, bunkList, grade) {
        for (const bunk of bunkList) {
            ...
            for (const special of (sl.specials?.priorityList || [])) {
                ...
                // If special has a location, check field availability
                if (special.location) {

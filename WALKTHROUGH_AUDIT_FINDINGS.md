...
- **🔴 FN-13 (CONFIRMED) — auto solver overloads a capacity-limited / `same_division` special ACROSS GRADES:** the solved auto output put **"Slush"** (`same_division`, cap 2) on **5 bunks across 3 divisions** (Soloists+Quints+Majors) at the same slot (≈2:55pm). **De-confounding done:** I removed the camp's custom layers from `_current` and re-ran — the overload **PERSISTED** (so it is NOT the custom layer, and NOT my injected config). It occurs in the normal **"special" layer** window. The special-placement paths DO call `canAssignSpecialToGrade` (L4716/5071/5186/6232/…), yet the overload happens — so per-special capacity/sharing is **not being enforced across grades**. LIKELY CAUSE: the solver has **two duplicate special-placement subsystems** (`canAssignSpecialToGrade`+`registerSpecialAssignment` defined at BOTH L4545 *and* L5646, each with its own `globalSpecialUsage`) — if different grade passes use different trackers, cross-grade special capacity isn't enforced, so when several grades' special slots funnel to the same least-recent special it exceeds cap + crosses divisions. Only **"Slush"** overloaded (the rest of the schedule — sports on fields, other specials — was clean), consistent with a single contended special. Manual mode = 0 special violations. **FIX:** ensure a SINGLE shared `globalSpecialUsage` across all grade passes / the two subsystems, so `canAssignSpecialToGrade` sees every grade's placements. **NEEDS dev confirmation of the exact tracker-scoping.**
...
- **FN-13 reproduced across both full days** (06-08: Slush overload; 06-09: Neranitas overload) — a *different* contended special each day, same cross-grade special-capacity gap. On both days, **fields / sports / sharing / access / time-rules were CLEAN and intra-day repeats = 0** — so the only rule violation is FN-13's special overload. Cross-day rotation read was partial (couldn't assemble a clean week due to FN-14); the auto rotation engine is shared with manual (verified good) + prior-audited (#90).

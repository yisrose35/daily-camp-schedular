...
- **⚠️ Auto multi-day / rotation NOT cleanly verifiable on this camp:** the current-grade auto config is orphaned (keyed to old grades), and per-date auto generation was inconsistent (06-08 produced a full 35-bunk/268-block schedule; 06-09 produced only 2 bunks). A clean cross-day rotation read for auto requires **properly rebuilding the auto-layer config for the current grades first** (a prerequisite + finding). The auto rotation engine itself is shared with manual (verified) + prior-audited (#90).

...
//
// Called from campistry_link_admin.html's "Connect Stripe" button on a
// link_staff_accounts row. The caller's own Supabase session JWT is used to
// read/write that row through Supabase's normal RLS (owner/admin only) —
// this is a deliberate departure from the rest of the Stripe functions in
// this repo, which trust an unauthenticated campId from the request body.
// Minting a Connect account tied to a real bank account is higher-stakes
// than the existing billing flow, so we lean on real auth here instead.
//
...
serve(async (req) => {
  ...
  try {
    ...
    const returnUrl = `${origin}/campistry_link_admin.html?stripeReturn=1&accountId=${encodeURIComponent(accountId)}`;

...
//      (the whole balance: a balanceFloor limits a child's SPENDING, and a
//      refund goes to the parent, so it is not held back — TED-142;
...
serve(async (req) => {
  ...
  try {
    ...
    const walletAvailable = Math.max(0, round2(balance + releasedBack
    ...
    if (totalRefunded > 0 && totalRefunded >= walletAvailable - 0.004) {
      await pauseAutoReload(supabase, authedCampId, camperId, acctKey || String(camperName ?? ""), account);
    }

           ...
           // General activity slot or other schedulable block
            if (normalizedGA || item.type === 'slot' || GENERATOR_TYPES.includes(item.type)) {
                ...
                _targetBunks.forEach(bunk => {
                    ...
                    // ★★★ v17.12: SPLIT per-bunk blocks at superset slot boundaries ★★★
                    // When a per-bunk skeleton block spans multiple superset slots,
                    // create separate schedulable blocks for each slot so the solver
                    // assigns a different activity to each one.
                    if (item._bunk && slots.length > 1) {
                        const divSlots = window.divisionTimes?.[divName] || [];
                        slots.forEach(function(slotIdx) {
                            const superSlot = divSlots[slotIdx];
                            if (!superSlot) return;
                            var subStart = Math.max(sMin, superSlot.startMin);
                            var subEnd = Math.min(eMin, superSlot.endMin);
                            if (subEnd - subStart < 5) return; // skip tiny fragments

                            var subExisting = window.scheduleAssignments[bunk]?.[slotIdx];
                            if (subExisting && subExisting._bunkOverride) return;

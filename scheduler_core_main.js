               ...
               const _isFG = window.isFullGradeForDivision ? window.isFullGradeForDivision(activityLabel, divName) : (activityProperties[activityLabel]?.fullGrade || activityProperties[activityLabel]?._fullGrade);
                ...
                if (window.GlobalFieldLocks?.isFieldLocked(activityLabel, slots, divName)) {
                    ...
                    for (const alt of alternatives) {
                        ...
                        endTime: eMin,
                        slots,
                        // ★★★ AUTO BUILD: Propagate flags for solver ★★★
                        _durationStrict: item._durationStrict || false,
                        _autoGenerated: item._autoGenerated || false,
                        _suggestedActivity: item._suggestedActivity || null,
                        _sportOnly: item._sportOnly || false
                    });
...
// ★★★ INITIALIZE WITH DIVISION-SPECIFIC SLOT COUNTS ★★★
        ...
        Object.keys(divisions).forEach(divName => {
            const divSlots = window.divisionTimes?.[divName] || [];
            const perBunkSlots = divSlots._perBunkSlots || null;
            const defaultSlotCount = divSlots.length > 0 ? divSlots.length : (window.unifiedTimes || []).length;
        ...
        manualSkeleton.forEach(item => {
           ...
           const isPinnedType = item.type === 'pinned' || 
            ...
            // ★★★ v5.0: Per-bunk slot lookup for auto mode ★★★
            if (item._bunk && item._autoGenerated) {
                const _pbSlots = window.divisionTimes?.[divName]?._perBunkSlots;
                if (_pbSlots && _pbSlots[item._bunk]) {
                    const _bunkSlotArr = _pbSlots[item._bunk];
                    const _bunkExact = _bunkSlotArr.findIndex(s => s.startMin === sMin && s.endMin === eMin);
                    if (_bunkExact !== -1) {
                        const _bunk = item._bunk;
                        if (bunkList.includes(_bunk)) {
                            const _ex = window.scheduleAssignments[_bunk]?.[_bunkExact];
                            if (!_ex || !_ex._bunkOverride) {
                                schedulableSlotBlocks.push({
                                    divName, bunk: _bunk,
                                    event: normalizeGA(item.event) || item.event || '',
                                    type: 'slot', startTime: sMin, endTime: eMin,
                                    slots: [_bunkExact],
                                    _durationStrict: item._durationStrict || false,
                                    _autoGenerated: true,
                                    _suggestedActivity: item._suggestedActivity || null,
...
if (exactSlot !== -1) {
    ...
    for (let i = 0; i < bunkSlots.length; i++) {
        if (bunkSlots[i].startMin < eMin && bunkSlots[i].endMin > sMin) slots.push(i);
           ...
           // General activity slot or other schedulable block
            if (normalizedGA || item.type === 'slot' || GENERATOR_TYPES.includes(item.type)) {
                // ★★★ AUTO BUILD: Per-bunk blocks only apply to specified bunk ★★★
                const _targetBunks = item._bunk 
                    ? [item._bunk].filter(b => bunkList.includes(b)) 
                    : bunkList;
               _targetBunks.forEach(bunk => {
                    const existing = window.scheduleAssignments[bunk]?.[slots[0]];
                    if (existing && existing._bunkOverride) return;

                    // ★★★ v17.13: Per-bunk blocks are NOT split at superset boundaries ★★★
                    // Each bunk has its own slot array (via _perBunkSlots). The solver
                    // sees one block per bunk with that bunk's own slot index.
                    // No fragmentation — a 40min block stays 40min.
                    schedulableSlotBlocks.push({
                        divName: divName,
                        bunk: bunk,
                        event: normalizedGA || eventName,
        ...
        Object.entries(divisions).forEach(([divName, divData]) => {
           ...
           const divSlots = window.divisionTimes?.[divName] || [];
            const isPerBunk = !!divSlots._isPerBunk;
            
            // ★★★ v17.13: Per-bunk gap detection ★★★
            if (isPerBunk && divSlots._perBunkSlots) {
                // In per-bunk mode, check each bunk's own slots for gaps
                bunkList.forEach(bunk => {
                    const bunkSlots = divSlots._perBunkSlots[String(bunk)] || [];
                    bunkSlots.forEach((slot, slotIdx) => {
                        if (slot.type !== 'slot') return;
                        if (slot._splitHalf) return;
                        const slotStart = slot.startMin;
                        const slotEnd = slot.endMin;
                        const hasBlock = schedulableSlotBlocks.some(block =>
                            String(block.divName) === String(divName) &&
                            String(block.bunk) === String(bunk) &&
                        ...
                        if (!hasBlock) {
                            ...
                            schedulableSlotBlocks.push({
                                divName: String(divName), bunk,
                                event: 'General Activity Slot', type: 'slot',
                                startTime: slotStart, endTime: slotEnd, slots: [slotIdx],
                                _fromGapDetection: true, _durationStrict: false, _autoGenerated: false
                            });
                            gapBlocksAdded++;
                        }
                    });

...
              // Safely get the fieldName, handling potential object storage
              const
fieldName = typeof entry.field === 'string' 
                ? entry.field 
                : (entry.field?.name || 'Unknown Fixed Resource');
              const
...
            // --- FIX: Extract name safely if entry.field is an object instead of a string ---
            const fixedActivityName = typeof entry.field === 'object' && entry.field !== null
              ? (entry.field.name || 'Fixed Activity') // If it's an object, try to get .name
              : entry.field; // Otherwise, use the value as is (should be string)
              
           td.innerHTML = `<span
class="fixed-pill">${fixedActivityName}</span>`;
          } else if
(entry.field === "Special Activity Needed" && !entry.sport) {
           td.innerHTML = `<span
class="need-special-pill">${entry.field}</span>`;
          } else {

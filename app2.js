    ...
    for (const bunk of allBunksInDiv) {
      for (let s = 0; s < window.unifiedTimes.length; s++) { 
        ...
        if (assignedSpan === 0 && h2hGameCount[bunk] < 2) {
            assignedSpan = tryH2H(bunk, div, s, spanLen, allBunksInDiv, h2hActivities, fieldUsageBySlot, isActive, activityProperties, allSchedulableNames, h2hHistory, h2hGameCount);
        }
 ...
 */
function tryH2H(bunk, div, s, spanLen, allBunksInDiv, h2hActivities, fieldUsageBySlot, isActive, activityProperties, allSchedulableNames, h2hHistory, h2hGameCount) {
            ...
            if (canFit && oppCanFit && finalSpan > 0) {
                ...
                for (let k = 0; k < finalSpan; k++) {
                    ...
                    if (pickedField && allSchedulableNames.includes(pickedField)) { 
                        ...
                        fieldUsageBySlot[currentSlot][pickedField] = 2;
                    }
                }
...
function canActivityFit(bunk, div, s, spanLen, pickedField, fieldUsageBySlot, isActive, activityProperties, forceNoShare = false) {
    let canFit = true; // This was the typo
 ...
 */
function assignActivity(bunk, s, spanForThisPick, pick, fieldUsageBySlot, generalActivityHistory, allSchedulableNames) {
    const pickedField = fieldLabel(pick.field);
    const activityName = getActivityName(pick);

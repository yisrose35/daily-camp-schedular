...
// Leagues now sequentially search for the next available time slot if a preferred slot is blocked.
...
    // Find the first available slot using the full 'spanValid' check by scanning all possible slots (0 to length-1).
    const allSlots = Array.from({ length: unifiedTimes.length }, (_, i) => i);
    
    const chosenSlot = allSlots.find(s => {
      // 1. Must be a slot where the division is active
      if (!(divisionActiveRows[div] && divisionActiveRows[div].has(s))) return false;
      // 2. Must pass the full validity check (span length, fixed blocks, global league non-overlap)

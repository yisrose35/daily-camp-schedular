// ============================================================================
// midday_rain_stacker.js — Mid-Day Rain Schedule Stacking Engine (v1.0)
// ============================================================================
// 
...
console.log('🌧️ Mid-Day Rain Stacker v1.0 loading...');
...
function handleMidDayRainStart(rainStartMinutes, resourceOverrides) {
    ...
    // ★★★ Backup morning schedule assignments before rebuild ★★★
    ...
    Object.keys(schedules).forEach(bunk => {
        backup[bunk] = {};
        const bunkSlots = schedules[bunk] || [];
        bunkSlots.forEach((slot, idx) => {
            if (slot && times[idx]) {
                const slotEnd = new Date(times[idx].end);
                const slotEndMin = slotEnd.getHours() * 60 + slotEnd.getMinutes();
                if (slotEndMin <= rainStartMinutes) {
                    backup[bunk][idx] = JSON.parse(JSON.stringify(slot));
                    preservedCount++;
                }
            }
        });
    ...
    // ★★★ Trigger auto-regeneration to fill activity blocks with actual specials ★★★
    if (result.success) {
...
function handleMidDayRainClear(clearTimeMinutes, regularSkeletonName, resourceOverrides) {
    ...
    // ★★★ Backup current rainy schedule assignments before rebuild ★★★
    const times = window.unifiedTimes || [];
    const schedules = window.scheduleAssignments || {};
    const backup = {};
    let preservedCount = 0;
    ...
    Object.keys(schedules).forEach(bunk => {
        ...
        bunkSlots.forEach((slot, idx) => {
            if (slot && times[idx]) {
                const slotEnd = new Date(times[idx].end);
                const slotEndMin = slotEnd.getHours() * 60 + slotEnd.getMinutes();
                if (slotEndMin <= clearTimeMinutes) {
                    backup[bunk][idx] = JSON.parse(JSON.stringify(slot));
                    preservedCount++;
                }
            }
 ...
 * 
 * This preserves the morning's actual schedule assignments and only
 * regenerates for the blocks that were rebuilt.
 */
        ...
        try {
            ...
            if (typeof window.runSkeletonOptimizer === 'function') {
                ...
                if (skeleton.length > 0) {
                    ...
                    // Dispatch event so pinned preservation can capture before generation
                    ...
                    // After generation, restore preserved morning assignments
                    ...
                    console.log('[RainStacker] ✅ Post-rebuild generation complete');
                } else {
...
/**
 * Restore the preserved morning schedule assignments after regeneration.
 * The optimizer will have overwritten ALL assignments, so we need to
 * put the morning ones back.
 */
...
console.log('🌧️ Mid-Day Rain Stacker v1.0 loaded ✅');

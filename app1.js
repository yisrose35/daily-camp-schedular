...
// ===== START OF FIX 1: Re-added missing uid function =====
function uid() {
...
// ... (All Field/Special helper functions (renderAvailabilityControls, etc.) also go here) ...
// (These were in the previous file but omitted for brevity)
// ...

// -------------------- Bunks --------------------
// ===== START OF FIX 2: Re-added missing Bunk functions =====
function addBunk() {
...
function addDivision() {
    ...
    if (!availableDivisions.includes(name)) {
        ...
        // window.DailyActivities?.onDivisionsChanged?.(); // This was from an old file
        window.updateTable?.();
...
function setupDivisionButtons() {
    ...
    availableDivisions.forEach(name => {
        ...
        makeEditable(span, newName => {
            ...
            setupDivisionButtons();
            window.initLeaguesTab?.();
            // window.DailyActivities?.onDivisionsChanged?.(); // Old
            renderDivisionSkeletonEditor(); // NEW
            window.updateTable?.();
        });
...
// -------------------- Fields / Specials --------------------
// (Unchanged: addField, renderFields, addSpecial, renderSpecials, and their helpers)
// ...
...
function loadData() {
    ...
    try {
        ...
        if (Object.keys(divisionSkeletons).length === 0 && (data.divisions[availableDivisions[0]] && 'periodRules' in data.divisions[availableDivisions[0]])) {
...
window.getSpecials = () => specialActivities;// -------------------- State --------------------
let bunks = [];
let divisions = {}; // { divName:{ bunks:[], color } }

...
(function () {
    ...
    function injectStyle() {
        ...
        '#' + WRAP_ID + ' .cw-reason{padding:2px 12px 6px;}' +
        '#' + WRAP_ID + ' .cw-reason .cw-why{font-size:11.5px;color:#6b7280;line-height:1.4;margin:0 0 5px;}' +
        '#' + WRAP_ID + ' .cw-bunks{display:flex;flex-wrap:wrap;gap:4px;}' +
    ...
    function buildBody() {
        // group: activity → reason → [bunk labels]
        var byAct = {};
        _items.forEach(function (it) {
            var a = it.activity || 'Custom';
            if (!byAct[a]) byAct[a] = { count: 0, reasons: {} };
            byAct[a].count++;
            var r = it.reason || '';
            if (!byAct[a].reasons[r]) byAct[a].reasons[r] = [];
            var label = it.grade ? (it.bunk + ' (' + it.grade + ')') : String(it.bunk);
            byAct[a].reasons[r].push(label);
        });
        var html = '';
        Object.keys(byAct).forEach(function (a) {
            ...
            Object.keys(grp.reasons).forEach(function (r) {
                html += '<div class="cw-reason">';
                if (r) html += '<div class="cw-why">' + esc(r) + '</div>';
                html += '<div class="cw-bunks">';
                grp.reasons[r].forEach(function (lbl) { html += '<span class="cw-bunk">' + esc(lbl) + '</span>'; });
                html += '</div></div>';
            });
        });
        return html;
        ...
        if (!_open) {
            ...
            wrap.innerHTML =
                '<div class="cw-panel">' +
                    ...
                    '<div class="cw-foot">These bunks didn’t get the activity for this day — adjust the config, or accept and move on.</div>' +
                '</div>';

...
| **If it fails** | `renderBilling` (14345), `buildFamilyLedgers` (12885), `setBillFilter` (14696). |
| **Sev** | S2 |
...
| **If it fails** | `renderFamilyDetailPage` (14495), `_installmentTableHtml` (14674), `_collectionWarning` (14284). |
...
| **If it fails** | `addCharge` (14762), `addChargeForFamily` (14765), `campistry_billing_core.js`. |
| **Sev** | S1 |
...
| **If it fails** | `openPaymentModal` (14706), `_postPaymentEntry` (3785), `campistry_payments.js`. |
| **Sev** | S1 |
...
| **If it fails** | `removePayment` (15360). |
| **Sev** | S1 |
...
| **If it fails** | `issueCredit` (15035), `_famRefundablePayments` (14805), `_crUpdateRefundSummary` (14887). |
| **Sev** | S1 |
...
| **If it fails** | `REFUND_WINDOW_DAYS` (~14663), `_famRefundableOnline` (14847), `_famRefundExpired` (14867). |
| **Sev** | S1 |
...
| **If it fails** | `printStatement` (15188). |
| **Sev** | S3 |
...
| **If it fails** | `printTaxStatement` (15260), `campistry_tax_statement.js`, `tests/tax_statement.test.js`. |
...
| **If it fails** | `requestCardSetup` (15475), `card-capture-start` / `payments-save-method`, `campistry_card_setup.html`, `get_camp_public_tokenization_key`, migrations 128/136/139/151. |
| **Sev** | S1 |
...
| **If it fails** | `chargeStoredCard` (15540), `charge-saved-card`. |
| **Sev** | S1 |
...
| **If it fails** | `batchCharge` (15629), `175_chargebacks_and_collection_blocks.sql`. |
| **Sev** | S1 |
...
| **If it fails** | `sendPayLink` (15680), `payments-hosted-link` / `payments-hosted-complete`. |
| **Sev** | S1 |
...
| **If it fails** | `monthlyPlan` (15915), `_mpGenRows` (15810), `_mpStepDate` (15801), `_buildInstallmentSchedule` (11968), migrations 115-118, 181. |
| **Sev** | S1 |
...
| **If it fails** | `toggleFamilyAutopay` (16050), `charge-due-installments`. |
| **Sev** | S1 |
...
| **If it fails** | `cancelMonthlyPlan` (16059). |
| **Sev** | S1 |
...
| **If it fails** | `campistry_card_fees.js`, `_cfCardHtml` (16357), `tests/card_fees.test.js`, migration `192_card_fee_policy.sql`. |
...
| **If it fails** | `addScholarship` (18498), `tests/aid_and_credits_reach_the_ledger.test.js`. |
| **Sev** | S1 |
...
| **If it fails** | `campistry_deposit_policy.js`, `_depPolicy` (16143), `tests/deposit_policy.test.js`, migration `164_public_deposit_policy.sql`. |
| **Sev** | S2 |
...
| **If it fails** | `markDepositPaid` (16179), `chargeDepositNow` (16211). |
| **Sev** | S1 |
...
| **If it fails** | `openUnmatchedPaymentsModal` (14966). |
| **Sev** | S2 |
...
| **If it fails** | `prEditStaff` (13797), `prToggleYc` (13916), `prPayTypeHint` (13920). |
| **Sev** | S2 |
...
| **If it fails** | `_prTimesheetsTab` (13955), `prSetHours` (14011), `_prWeekStart` (13265), `_prShiftWeek` (13271). |
| **Sev** | S2 |
...
| **If it fails** | `campistry_payroll_core.js`, `_prYouthTab` (14048), `tests/payroll_core.test.js`. |
...
| **If it fails** | `prNewRun` (14190), `prExportCSV` (14230), `prDeleteRun` (14217). |
| **Sev** | S2 |
...
| **If it fails** | `ptConnectStripe` (13454), `ptPayout` (13504), `_ptCheckStripeReturn` (13475), `TIPPING_SETUP.md`. |
| **Sev** | S1 |
...
| **If it fails** | `_ptParseCsv` (13622), `ptUploadTemplate` (13635), `ptPayout` (13504). |
| **Sev** | S1 |
...
| **If it fails** | `renderFinance` (12063), `finSetTab` (12458), `finSetBudget` (12680). |
| **Sev** | S2 |
...
| **If it fails** | `finReconcileCharges` (12469), migration `162_reconcile_processor_charges.sql`. |
| **Sev** | S1 |
...
| **If it fails** | `finExportCSV` (12695) through `finExportJournal` (12779), `fmtIIFDate` (12841). |
| **Sev** | S2 |
...
| **If it fails** | `finImportCSV` (12806). |
| **Sev** | S1 |

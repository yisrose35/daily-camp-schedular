...
## Last commit checked
`4010f67` (2026-09-24, billing eleventh pass)

## Open findings
| ID | Severity | Description | Found | Status |
|----|----------|-------------|-------|--------|
| TED-124 | 🔴 | (pre-existing since Sep 8) Snacks → Refund All does nothing: `_stripeRefundCapacity` is not defined (`campistry_snacks.js:2093`), ReferenceError, window never opens. Also hides the Refund All "waiting" list, and a Stripe hold for a child at $0 can never be looked up | 2026-09-24 | Open |
| TED-116 | 🟠 | Server actions (holds / resolveHold) now work on real SQL, but the Snacks "waiting for an answer" box crashes (`_lbl` is defined inside `getCamperList`), and Refund All's window can't open (TED-124) → still no screen to record "it went through"; a child whose lost refund DID go through is blocked from further refunds unless the office wrongly presses OK ("not there") | 2026-09-24 | Open (server fixed, page broken) |
| TED-126 | 🟠 | Stripe refunds that fail after being accepted are never noticed (no refund.failed / charge.refund.updated handling; money returns to the platform balance, ledger/wallet say refunded). New in 4010f67: a stuck canteen refund found "failed" is re-sent with the same key within 24 h → Stripe replays the old "succeeded" → booked, wallet $0, parent $0 | 2026-09-24 | Open |
| TED-120 | 🟠 | Narrowed at 4010f67: `ch_` now refused, but "went through" with ANOTHER family's same-amount `pi_` (or a typo'd `pi_`) → family credited twice, next instalment "nothing owed" (276 checks only a different amount) | 2026-09-24 | Open (partly fixed) |
| TED-125 | 🟡 | (pre-existing since Sep 23, 2330ff9) Take Out Cash: payout recorded, then ReferenceError `_lbl` → no "Paid out" message, amount left in the box → risk of recording it twice | 2026-09-24 | Open |
| TED-127 | 🟡 | Stripe single refund after the look-up settles a stuck refund: deposit maths uses the pre-settle snapshot → "Charge … has already been refunded", nothing refunded; second press works. Money right | 2026-09-24 | Open |
| TED-128 | 🟡 | BILLING_PAYMENTS_SETUP.md's new Dashboard steps omit "Enforce JWT Verification OFF" for charge-due-installments (BYOP_SETUP.md has it); a fresh deploy per the billing guide → nightly call refused, nobody charged | 2026-09-24 | Open |
| TED-005 | 🟠 | 14 auto-scheduler tests fail (`auto_full_day.test.js`); still 14 at 4010f67. Owner deferred. | 2026-09-23 | Open (deferred by owner) |
...
| Billing & payments (edge functions, autopay runner, refunds, late fees/surcharges/credits, plans, parent balance) | 2026-09-24 (eleventh pass: TED-117/118/119/121/122/123 closed; 81 earlier probes re-run; faithful pretend Stripe (key replay + param check, 24 h forget, refund list with metadata, failed refunds) on real SQL; Cardknox hold actions on real SQL; nightly run chained; wrong-row autopay answer; Snacks refund windows + cash-out in a real browser. Not live processors; surcharge refunds, photos, tips, payroll never) |
| Payroll | never |
...
| Canteen / Snacks / Shop / POS | canteen refunds on real SQL (275 holds, Stripe look-up, Cardknox hold actions) 2026-09-24 11th pass; Snacks Refund, Refund All and Take Out Cash clicked in a real browser 2026-09-24 (TED-124/125 found); auto-reload, shop bill-to-family earlier; POS maths never |

// Re-run of TED-065 against today's _postExistingCharges (run twice = two page loads).
...
const db = require(R + '/tests/e2e/db.js').boot({ port: 5475 });
const CAMP = 'a1a1a1a1-a1a1-a1a1-a1a1-a1a1a1a1a4a1', OWNER = 'b1b1b1b1-b1b1-b1b1-b1b1-b1b1b1b1b1b1';
try {
  ...
  const grab = name => { const at = src.indexOf('function ' + name + '('); let i = src.indexOf('{', at), d = 0; for (; i < src.length; i++) { if (src[i] === '{') d++; else if (src[i] === '}' && --d === 0) break; } return src.slice(at, i + 1); };
  ...
  const posted = [ctx._postExistingCharges(fam), ctx._postExistingCharges(fam)];

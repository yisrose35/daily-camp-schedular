// Probe (23rd pass copy of moveback22; S5 rewritten for TED-209)
...
const O = path.join(R, 'ted/probes/2026-09-24-billing-23/moveback23');
...
(async () => {
  ...
  try {
    ...
    log('S5  (TED-209) the kept "$300 — the part … paid" line on the fund\'s account');
    await A.evaluate(() => window.CampistryMe.payerLines('org_scholarship_fund')); await wait(600);
    const rows5 = await A.evaluate(() => Array.from(document.querySelectorAll('#dynModal tr')).map(r => ({ t: r.textContent.replace(/\s+/g, ' ').trim(), btns: Array.from(r.querySelectorAll('button')).map(b => b.textContent.trim()) })));
    log(`  account rows: ${JSON.stringify(rows5)}`);
    const kept = rows5.find(r => /the part Scholarship Fund paid/.test(r.t));
    check('S5 the fully paid kept share shows "paid", not Move back', !!kept && /paid$/.test(kept.t) && !kept.btns.includes('Move back to family'), JSON.stringify(kept));
    check('S5 the unpaid shares still offer Move back (none left here → rows with Move back only where unpaid)', rows5.filter(r => r.btns.includes('Move back to family')).every(r => !/the part Scholarship Fund paid/.test(r.t)), '');
    check('S5 no doubled title anywhere', rows5.every(r => !/paid — the part/.test(r.t)), '');
    await A.evaluate(() => window.closeModal ? window.closeModal('dynModal') : null).catch(() => {});
    const L5 = JSON.parse(pl('pine'));
    const keepId = (L5.find(e => /^prkeep_/.test(e.id)) || {}).id;
    const before5 = pl('pine');
    log(`  a window left open from before (or a double click) presses Move back on ${keepId} anyway`);
    await A.evaluate((id) => window.CampistryMe.voidPayerLine('org_scholarship_fund', id), keepId); await wait(1500);
    const d5 = await dlg(A);
    const t5 = await toastLast(A);
    log(`  confirm window: "${d5.slice(0, 160)}"; toast "${t5}"`);
    check('S5 pressing it anyway writes nothing and says so', pl('pine') === before5 && /Nothing to move back/.test(t5), t5);
    log(`  Pine owes $${owes('pine')}; fund ${JSON.stringify(fund())}`);

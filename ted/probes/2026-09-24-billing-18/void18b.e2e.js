...
//   (void18b: after pressing Void, the office closes History with its own Close button, then clicks Void sale)
// Run: node ted/probes/2026-09-24-billing-18/void18b.e2e.js
'use strict';
...
(async () => {
  ...
  try {
    ...
    await page.screenshot({ path: path.join(__dirname, 'void18b_window.png') });
    await page.click('#m-history .modal-footer button', { timeout: 5000 });
    await wait(500);
    await page.screenshot({ path: path.join(__dirname, 'void18b_after_close.png') });
    let clicked = true;
    await page.click('#voidBtn', { timeout: 5000 }).catch((e) => { clicked = false; log('    still no real click: ' + String(e.message).split('\n')[0]); });
    log('    after closing History, a real click on "Void sale" worked: ' + clicked);
    if (!clicked) await page.evaluate(() => document.getElementById('voidBtn').click());
    await wait(3000);

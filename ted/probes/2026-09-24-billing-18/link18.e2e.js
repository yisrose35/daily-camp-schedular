// Probe (18th pass copy of link17 — also lifts _arKey and _arPendingOn, which the TED-172 fix added;, TED-161 re-check + edges). Link's Auto-Reload card — its
...
// Run: node ted/probes/2026-09-24-billing-18/link18.e2e.js
'use strict';
...
(async () => {
  ...
  try {
    ...
    const js = ['_escHtml', '_arToggleOption', '_arRenderDayOptions', '_arRenderSessionPicker', '_renderAutoReload', 'saveAutoReload', 'turnOffAutoReload', '_saveAutoReloadConfig', '_arAutoSave', '_arKey'].map(cut).join('\n');
    ...
    await page.setContent(`<html><head><style>.ar-option-body{display:block}</style></head><body>${card}<script>
      ...
      var _arSaveTimer = null; var _arPendingOn = {};
      ${js}

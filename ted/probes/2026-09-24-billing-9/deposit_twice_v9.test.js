// 9th pass copy of probes/…-5/deposit_twice.test.js: loader also cuts _markCardPayment (new in 66a65df), nothing else changed.
...
test('probe', () => {
    for (const [label, manual] of [
        ...
        console.log(`# ${label}: balance before ${before} -> after Billing load ${B.balance(f)} (should stay 750); payment rows ${ctx.finPayments.length}`);
    }
});

// 9th pass copy of tests/edge_harness.js (Ted): loads SEVERAL real edge functions
// into one process so they can race each other. T.handlers[k] is function k.
// =============================================================================
...
const ROOT = process.env.TED_ROOT || '/home/user/daily-camp-schedular';   // TED_ROOT: run against an older checkout
...
function runEdges(names, scenario, opts) {
    const name = names[0];
    opts = opts || {};
    const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'edges-'));
    names.forEach((nm, k) => {
    const src = fs.readFileSync(path.join(ROOT, 'supabase/functions', nm, 'index.ts'), 'utf8');
    let fn = src

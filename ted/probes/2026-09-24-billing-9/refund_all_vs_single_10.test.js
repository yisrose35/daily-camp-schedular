// 9th pass variant: the single refund is $10 (not the whole $20) — different amounts never shared a key, even before 66a65df.
...
const singleBody = { camperId: 7, camperName: 'Avi', amount: 10, idempotencyKey: 'cref_b' };
...
for (const [proc, scen, single, all] of [['Stripe', stripe, 'stripe-canteen-refund', 'stripe-canteen-refund-all'], ['Cardknox', cardknox, 'payments-canteen-refund', 'payments-canteen-refund-all']]) {
  for (const timing of ['single-first', 'all-first']) {
    test(`${proc} ${timing}`, () => {
      ...
      console.log('# ' + line);
      assert.ok(moved <= 20.0001 && true, 'more than the $20 left in the wallet went back: ' + line);
    });
  }
}

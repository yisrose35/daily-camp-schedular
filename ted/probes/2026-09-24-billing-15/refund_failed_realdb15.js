...
// 15th pass copy: also lifts the new _famPaymentsIn helper (and gives it families).
// Run: node ted/probes/2026-09-24-billing-15/refund_failed_realdb15.js  (14th pass: 13th-pass probe + claim_refund_failure_alert on the real DB)
'use strict';
...
try {
  ...
  });
  {
    ...
    const ctx = { finPayments, families: { gold: fam }, normalizePersonId: () => null, camperNameById: () => null, Date, Math, String, Number };
    vm.createContext(ctx);
    vm.runInContext(['_famPaymentsIn', '_famRefundablePayments', '_refundedFrom', '_paymentAgeDays', '_famRefundableOnlineAll', '_famRefundableOnline'].map(cut).join('\n') +

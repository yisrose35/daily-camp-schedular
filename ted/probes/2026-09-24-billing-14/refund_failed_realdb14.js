...
// 13th pass copy: T1b now lifts _refundedFrom too (new page helper, TED-132);
// W3 prints the office's notice; section N counts platform alert emails for a
// failed refund that is booked / not booked / has no camp, over re-deliveries.
// Run: node ted/probes/2026-09-24-billing-14/refund_failed_realdb14.js  (14th pass: 13th-pass probe + claim_refund_failure_alert on the real DB)
'use strict';
...
const db = require(R + '/tests/e2e/db.js').boot({ port: 5643 });
...
try {
  ...
  }
  {
    ...
    const h1 = hook(c, ev('refund.failed', refundNow(re)));
    const sMid = state(c);
    console.log(`    after the webhook: wallet $${sMid.wallet}, holds [${sMid.holds}], history [${sMid.lines}]  (${hookSay(h1)})`);
    console.log(`    the office's notice: "${q1(`SELECT coalesce(max(title || ': ' || body), '') FROM notifications WHERE camp_id='${c.C}' AND source='refund_failed'`)}"`);
    const hR = [hook(c, ev('refund.failed', refundNow(re))), hook(c, ev('refund.updated', refundNow(re))), hook(c, ev('charge.refund.updated', refundNow(re)))];
    console.log(`    3 more deliveries of the same failure → emails ${hR.reduce((n, h) => n + h.emails.length, 0)}, notices ${state(c).notices}`);
    const a2 = run(ALL, c, {});
  ...
  // ── N. TED-131: platform alerts, counted over re-deliveries ──────────────
  {
    console.log('N. A failed refund whose payment has NO camp stamp and no camp anywhere (e.g. a payment made outside Campistry): refund.failed, then Stripe re-sends it, then refund.updated and charge.refund.updated');
    const c = camp([20], 0); clearFlags();
    run(ALL, c, {});
    const re = failRefund(c, 'last');
    const obj = refundNow(re); obj.metadata = {}; obj.payment_intent = 'pi_elsewhere'; obj.charge = 'ch_elsewhere';
    const hs = [hook(c, ev('refund.failed', obj)), hook(c, ev('refund.failed', obj)), hook(c, ev('refund.updated', obj)), hook(c, ev('charge.refund.updated', obj))];
    console.log(`    HTTP ${hs.map(h => h.status).join('/')}; emails per delivery ${hs.map(h => h.emails.length).join('/')}: ${JSON.stringify(hs[0].emails)}; last log "${(hs[0].logs.slice(-1)[0] || '').slice(0, 120)}"`);
  }
  ...
  });
  {
    ...
    vm.runInContext(['_famRefundablePayments', '_refundedFrom', '_paymentAgeDays', '_famRefundableOnlineAll', '_famRefundableOnline'].map(cut).join('\n') +

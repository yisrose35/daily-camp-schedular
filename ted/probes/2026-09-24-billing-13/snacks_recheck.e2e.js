...
//       Refund All.
// Run: node ted/probes/2026-09-24-billing-13/snacks_recheck.e2e.js
'use strict';
...
${STRIPE}`;
  ...
  try {
    ...
    // ── R1 ──
    const d1 = topUp('Avi Katz', 'pi_OLD1', 30, 40);
    log(`\nR1. Avi Katz's parent topped up $30 by card on ${d1} (40 days ago). Wallet $${wallet('Avi Katz')}.`);
    await openSnacks();
    let w = await openRefundFor('Avi Katz');
    log(`    Refund window: "${w.box.slice(0, 150)}" | amount "${w.amt}" | button off: ${w.off}`);
    check('R1a the window offers the 40-day-old $30', /Stripe: \$30\.00/.test(w.box) && w.amt === '30.00' && !w.off, '');
    await page.evaluate(() => { window.confirm = () => false; });
    await page.click('#refundBtn');
    await waitFor('the answer', () => page.evaluate(() => !window._canteenRefundBusy), 20000);
    await new Promise(r => setTimeout(r, 1500));
    log(`    after pressing Refund: toast ${JSON.stringify((await toasts()).slice(-1))}, Stripe [${stripeMade()}], wallet $${wallet('Avi Katz')}`);
    check('R1b the parent gets the $30 back once, wallet $40 (the cash) left', stripeBack('pi_OLD1') === 30 && wallet('Avi Katz') === 40, '');

    // ── R2 ──
    const d2 = topUp('Avi Stern', 'pi_OLD2', 25, 35);
    topUp('Avi Katz', 'pi_OLD3', 15, 20);
    log(`\nR2. Avi Stern topped up $25 on ${d2}; Avi Katz topped up $15 twenty days ago. Refund All:`);
    await openSnacks();
    let ra = await openRefundAll();
    log(`    window: "${ra.body.slice(0, 120)}" | button ${JSON.stringify(ra.btn)}`);
    check('R2a Refund All offers both children, $40.00', ra.btn === 'Refund All ($40.00)' && /2 campers/.test(ra.body), '');
    let res = await runRefundAll();
    log(`    result: "${res}" → Stripe [${stripeMade()}]; wallets Katz $${wallet('Avi Katz')}, Stern $${wallet('Avi Stern')}`);
    check('R2b each child refunded from their own top-up: Stern $25 (pi_OLD2), Katz $15 (pi_OLD3); wallets $40 / $0',
      stripeBack('pi_OLD2') === 25 && stripeBack('pi_OLD3') === 15 && wallet('Avi Katz') === 40 && wallet('Avi Stern') === 0 && /Refunded \$40\.00 across 2 campers/.test(res), '');

    // ── R3 ──
    topUp('Avi Katz', 'pi_NEW4', 20, 0);
    log(`\nR3. Avi Katz tops up $20 today (pi_NEW4). The office refunds $20; Stripe makes it but the answer is lost; the office cancels the question`);
    await openSnacks();
    w = await openRefundFor('Avi Katz');
    ...
    log(`    wallet $${wallet('Avi Katz')}, holds [${holdsDb()}], Stripe [${stripeMade()}]`);
    await openSnacks();
    ra = await openRefundAll();
    log(`    Refund All window: "${ra.body.slice(0, 100)}" | waiting "${ra.holds.slice(0, 110)}" | button ${JSON.stringify(ra.btn)}`);
    res = ra.btn ? await runRefundAll() : '(no button)';
    log(`    result: "${res}" → holds [${holdsDb()}], Stripe [${stripeMade()}], wallet $${wallet('Avi Katz')}`);
    check('R3 the result says the waiting $20 had gone through and is recorded; nothing sent twice',
      /1 waiting refund \(\$20\.00\) had gone through and is now recorded/.test(res) && stripeBack('pi_NEW4') === 20 && /posted \$20\.00$/.test(holdsDb()), '');

    // ── R4 ──
    topUp('Avi Katz', 'pi_NEW5', 20, 0);
    log(`\nR4. Avi Katz tops up $20 (pi_NEW5). The office refunds $20; the request never reaches Stripe; the office cancels the question`);
    await openSnacks();
    w = await openRefundFor('Avi Katz');
    await page.fill('#refundAmt', '20'); await page.evaluate(() => refundAmtChanged());
    flag('lose_before');
    ...
    res = ra.btn ? await runRefundAll() : '(no button)';
    log(`    result: "${res}" → holds [${holdsDb()}], Stripe [${stripeMade()}], wallet $${wallet('Avi Katz')}`);
    check('R4 the result says the waiting $20 had not gone through, and it really was refunded (pi_NEW5 $20 back once, wallet $40)',
      /had not gone through/.test(res) && stripeBack('pi_NEW5') === 20 && wallet('Avi Katz') === 40, '');

// 16th-pass corrected copy: lifts the new cash-discount helpers openPaymentForFamily calls (no other change).
...
test('probe', () => {
  ...
  const ctx = {
    ...
    document: { getElementById: (id) => ({ value: fields[id], addEventListener() {}, innerHTML: '' }) }, Date,
    _cfAPI: () => null, enrollSettings: {}, _postLedgerCredit: () => false,
  };
  vm.createContext(ctx);
  // 16th pass: openPaymentForFamily now also calls the cash-discount helpers (TED-153); lifted with it.
  const VARS = ME.match(/var _CASH_DISCOUNT_METHODS=\{[^}]*\};/)[0];
  vm.runInContext(VARS + '\n' + ['_camperIdOf', '_paymentRefOf', '_paymentRefsOf', '_postPaymentEntry', '_cashDiscountFor', '_postCashDiscount', '_payDiscountPreview', 'openPaymentForFamily'].map(cut).join('\n') + '\nthis.open=openPaymentForFamily;', ctx);

// 16th-pass corrected copy of the 15th-pass probe: the only change is that each
// camp now has a session covering today (TED-143: auto-reload charges only on
// days camp is in session), so the P5 control can charge again.
...
function camp() {
  ...
  db.sql(`INSERT INTO camps (id, owner, name, payment_processor_key) VALUES ('${C}','${OWNER}','S${n}','cardknox');
    ...
    INSERT INTO camp_state_kv (camp_id, key, value) VALUES ('${C}', 'campistryMe', jsonb_build_object('sessions', jsonb_build_array(jsonb_build_object('id','s1','name','Summer','startDate', to_char(now() - interval '10 days','YYYY-MM-DD'),'endDate', to_char(now() + interval '10 days','YYYY-MM-DD')))));`);
  return C;
}
